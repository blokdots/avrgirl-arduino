...
Connection.prototype._init = function (callback) {
  ...
  if (!_this.options.port) {
    ...
    _this._sniffPort(function (error, port) {
      if (error) {
        var error = new Error(
          "an unexpected error happened when connecting to board: " +
            error.toString()
        );
      ...
      if (port && port.length) {
        ...
        _this.debug(
          "found " + _this.options.board.name + " on port " + _this.options.port
        );
        ...
        return callback(
          new Error(
            "no Arduino " + "'" + _this.options.board.name + "'" + " found."
          )
        );
      }
    });
